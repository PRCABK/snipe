...
| M1 | Partial | Single-instance, tray, hotkeys, display/window enumeration, GDI capture, clipboard, encoders, and basic settings code exist. The mutex/pipe names are not SID/session scoped, the pipe has no explicit current-user ACL or protocol limits, hotkey conflicts are only logged, saved hotkeys are not re-registered, settings do not control startup, and the screenshot overlay is not wired to the captured image or physical desktop geometry. Window selection and resize handles are absent. |
| M2 | Partial library only | Annotation models, a limited command stack, CPU rasterization, and a toolbar component exist. There is no editor window/canvas/controller integration. Selection, move/resize, delete/copy, crop, text editing/IME, complete styling, and end-to-end export are absent. The overlay's `action-annotate` callback is not connected. |
...
| M5 | Partial library only | A frame session type, overlap estimator, safer frame validation, and vertical stitcher exist. Hotkey/tray longshot commands currently invoke ordinary screenshots. Region selection, capture loop, manual/automatic scrolling, stability/end detection, fixed-header handling, preview/recovery, resource budgets, and the 30-sample test corpus are absent. |
| M6 | Not complete | A minimal current-user Inno installer and release workflow exist. Signing, RFC 3161 timestamping, license bundle, accessibility/i18n integration, diagnostics, install/upgrade/uninstall lifecycle tests, and Windows 10/11 clean-install evidence are absent. Upgrade IPC does not implement `ready`, `busy`, `cancelled`, or `incompatible`, and does not protect active work. Uninstall does not offer verified configuration/credential cleanup. |
