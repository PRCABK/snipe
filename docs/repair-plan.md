...
- [ ] Overlay 显示的是本次捕获帧，而不是纯色遮罩。
- [ ] 选区裁剪结果与屏幕像素误差 ≤ 1 物理像素（主屏 100%/150%/200%）。
- [ ] 副屏在主屏左侧（负坐标）时裁剪正确。
- [ ] 覆盖层、工具栏不进入最终图片。
- [ ] Esc / 取消隐藏 overlay，不退出进程；`current_frame` 在结束后释放。
...
- [ ] Ctrl+Alt+S 与托盘「滚动长截图」进入长图流程，而不是普通截图。
- [ ] 用户框选区域后可手动滚动采集，画面稳定后采帧。
- [ ] 拼接成功产出一张图，可复制/保存。
- [ ] 失败或达上限时不丢已采集帧，给出原因。
- [ ] 取消后回到托盘驻留。
...
- [ ] 截图工具栏「标注」打开编辑器，显示选区原图。
- [ ] 能添加至少矩形/箭头/画笔/马赛克，撤销/重做有效。
- [ ] 完成导出为原图像素分辨率，不按窗口缩放尺寸。
- [ ] 取消不修改剪贴板，回到驻留。
...
- [ ] 空闲时升级命令使主实例优雅退出并回复 `ready`。
- [ ] 长图/标注进行中回复 `busy`，不退出。
- [ ] 未知命令被拒绝，不执行。
- [ ] 安装器等待超时有明确失败，而不是直接覆盖正在写的文件。
